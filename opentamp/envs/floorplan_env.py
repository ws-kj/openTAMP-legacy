...
class FloorplanEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for _ in range(10):
            # if np.abs(self.position[0] // 1 - (self.position[0] + small_incr[0])//1) > 0.001:
            #     square_idx_old = int((self.position[0] // 1) * self.num_columns + (self.position[1] // 1))
            #     square_idx_new = int(((self.position[0] + small_incr[0]) // 1) * self.num_columns + ((self.position[1] + small_incr[1]) // 1))
            #     if self.grid_graph[square_idx_new] in self.grid_graph[square_idx_old].neighbors:
            #         self.position += small_incr
            #     elif self.grid_graph[square_idx_new] in self.grid_graph[square_idx_old].door_neighbors:
            #         if 1/3 < (self.position[1] % 1) < 2/3 and 1/3 < ((self.position[1] + small_incr[1]) % 1) < 2/3:
            #             self.position += small_incr
            #         else:
            #             break
            #     else:
            #         break
            # elif np.abs(self.position[1] // 1 - (self.position[1] + small_incr[1])//1) > 0.001:
            #     square_idx_old = int((self.position[0] // 1) * self.num_columns + (self.position[1] // 1))
            #     square_idx_new = int(((self.position[0] + small_incr[0]) // 1) * self.num_columns + ((self.position[1] + small_incr[1]) // 1))
            #     if self.grid_graph[square_idx_new] in self.grid_graph[square_idx_old].neighbors:
            #         self.position += small_incr
            #     elif self.grid_graph[square_idx_new] in self.grid_graph[square_idx_old].door_neighbors:
            #         if 1/3 < (self.position[0] % 1) < 2/3 and 1/3 < ((self.position[0] + small_incr[0]) % 1) < 2/3:
            #             self.position += small_incrs
            #         else:
            #             break
            #     else:
            #         break
    ...
    def reset(self, seed=None):
        ...
        # self.walls.append((np.array([2.5, 0.]), np.array([2.5, self.door_loc-0.25])))
        # self.walls.append((np.array([2.5, self.door_loc+0.25]), np.array([2.5, 5.])))
...
class FloorplanEnvWrapper(FloorplanEnv):
    ...
    def get_prim_choices(self, task_list=None):
        out = OrderedDict({})
        out[utils.TASK_ENUM] = sorted(['move_near_loc_and_observe', 'view_vantage', 'zzzzz', 'approach_vantage', 'approach_targ', 'move_through_door'])
