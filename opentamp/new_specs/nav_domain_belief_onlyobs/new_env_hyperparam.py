...
def sample_fill_method(path, plan, agent, x0):
    ...
    for a_num in range(len(plan.actions)):
        # if plan.actions[a_num].name != 'observe_at_vantage' and a_num > 0:
...
def skolem_populate_fcn(plan):
    ...
    #     if plan.actions[a_idx].name == 'move_avoid_to_end':
    #         idx = a_idx
    ...
    print(plan.actions[plan.start].active_timesteps[0])
    ...
    for i in range(plan.actions[plan.start].active_timesteps[0], plan.horizon):
        if np.isnan(plan.params['pr2'].pose[:, i]).any():
            ...
            # iterate over all obstacles, break if any one of them
            is_collide = False
            for j in range(num_obstacles):
                obs_id = 'obs'+str(j+1)
                for samp_idx in range(plan.params[obs_id].belief.samples.shape[0]):
                    dist = np.linalg.norm(plan.params[obs_id].belief.samples[samp_idx,:,belief_ts] - plan.params['pr2'].pose[:, i])
                    if dist < 3.0 and -4.0 < plan.params['pr2'].pose[1, i] < 4.0:
                        is_collide = True
                        break
            if is_collide:
                break
...
def refresh_config(no=NUM_OBJS, nt=NUM_TARGS):
    ...
    config = {
        ...
        'sensor_dims': {
                ...
                utils.PAST_TASK_ARR_ENUM: 40,
                utils.PAST_MJCOBS_ARR_ENUM: 40 * GymEnvNavWrapper().observation_space.shape[0],
