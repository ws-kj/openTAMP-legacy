...
from opentamp.policy_hooks.rnn_gym_mj_spot_domain_agent import RnnGymMJSpotDomainAgent
import opentamp.new_specs.MJ_spot_domain.gym_prob as prob
from opentamp.policy_hooks.utils.file_utils import LOG_DIR
from opentamp.policy_hooks.observation_models import *

from opentamp.envs.SPOT_sim import SpotEnvWrapper
...
def refresh_config(no=NUM_OBJS, nt=NUM_TARGS):
    ...
    config = {
        ...
        'task_map_file': "policy_hooks/mjc_spot_mapping",
        'prob': prob,
        ...
        'num_targs': nt,
        'attr_map': ATTRMAP,
        'agent_type': RnnGymMJSpotDomainAgent,
        'gym_env_type': SpotEnvWrapper,
        'mp_solver_type': ToySolver,
        'll_solver_type': ToySolver,
        'meta_file': opentamp.__path__._path[0] + '/new_specs/MJ_spot_domain/MJ_spot_meta.json',
        'acts_file': opentamp.__path__._path[0] + '/new_specs/MJ_spot_domain/MJ_spot_acts.json',
        'prob_file': opentamp.__path__._path[0] + '/new_specs/MJ_spot_domain/MJ_spot_prob.json',
        'n_dirs': N_DIRS,
        ...
        'sensor_dims': {
                ...
                utils.PAST_MJCOBS_ARR_ENUM: 20 * SpotEnvWrapper().observation_space.shape[0],
                # utils.LIDAR_ENUM: N_DIRS,
                utils.MJC_SENSOR_ENUM: SpotEnvWrapper().observation_space.shape[0],
                utils.PAST_RECUR_ENUM: 20 + 20 * SpotEnvWrapper().observation_space.shape[0],
