...
class HLSearchNode(SearchNode):
    def __init__(self, abs_prob, domain, concr_prob, priority=0, prefix=None, label='', llnode=None, x0=None, targets=None, expansions=0, nodetype='optimal', info={}, replan_start=None, conditioned_obs={}, conditioned_targ={}, observation_model=None, belief_true={}, path=[]):
        ...
        self.conditioned_targ = conditioned_targ
...
class LLSearchNode(SearchNode):
    def __init__(self, plan_str, domain, prob, initial, priority=1, keep_failed=False, ref_plan=None, x0=None, targets=None, expansions=0, label='', refnode=None, freeze_ts=-1, hl=True, ref_traj=[], nodetype='optimal', env_state=None, info={}, replan_start=None, conditioned_obs={}, conditioned_targ={}, observation_model=None, belief_true={}, path=[]):
    ...
    def parse_state(self, plan, failed_preds, ts, all_preds=[]):
        ...
        for p in all_preds:
            new_preds.append({'pred': p, 'active_timesteps':(0,0), 'hl_info':'hl_state', 'negated': False})
        ...
        for a in plan.actions:
            ...
            for p in preds:
                ...
                check_ts = ts - p['active_timesteps'][1]

                if check_ts >= 0:
                    if p['pred'].__repr__().split(':')[1][1:] in [q['pred'].__repr__().split(':')[1][1:] for q in new_preds]:
                        if p['negated'] and p['pred'].__repr__().split(':')[1][1:] in [q['pred'].__repr__().split(':')[1][1:] for q in new_preds if not q['negated']]:
                                for q in new_preds:
                                    if q['pred'].__repr__().split(':')[1][1:] == p['pred'].__repr__().split(':')[1][1:]:
                                        new_preds.remove(q)

                        elif not p['negated'] and p['pred'].__repr__().split(':')[1][1:] in [q['pred'].__repr__().split(':')[1][1:] for q in new_preds if q['negated']]:
                                for q in new_preds:
                                    if q['pred'].__repr__().split(':')[1][1:] == p['pred'].__repr__().split(':')[1][1:]:
                                        new_preds.remove(q)
                        else:
                            ...
                            ## then was a repeat, don't consider this predicate
                ...
                if st <= ts and check_ts >= 0 and et >= st:
                    ...
                    elif not p['negated'] and p['pred'].hl_test(p['active_timesteps'][0], tol=1e-3):
                        new_preds.append(p)
                    elif p['negated'] and p['pred'].hl_test(p['active_timesteps'][0], tol=1e-3, negated=True):
                        new_preds.append(p)

        for pred in [p for p in failed_preds if p['pred'] is not None]:
            p = pred
            st, et = p['active_timesteps']
            check_ts = ts - p['active_timesteps'][1]
    ...
    def get_problem(self, i, failed_pred, failed_negated, suggester=None):
        ...
        preds = [{'pred': failed_pred, 'negated': not failed_negated, 'active_timesteps': (i, i), 'hl_info': 'hl_state'}]
